...
_I=alpha__("Y29udHJvbF9yZXNwb25zZQ==")
...
async def async_setup_entry(hass,entry,async_add_devices):
    ...
    if J:Q=[(C,next(iter(B[A][0].values())),B[D],B[E],B[F])for B in J for(C,B)in B.items()];R=[TISDaliLight(tis_api=B,light_name=C,brightness_channel=A,temperature_channel=A+100,device_id=D,gateway=E)for(C,A,D,F,E)in Q];C(R)
            ...
            if B.event_type==str(A.device_id):
                if B.data[_B]==_I:
                    ...
                    if B.data[_B]==alpha__("YmluYXJ5X2ZlZWRiYWNr"):
                        ...
                        if H[A.channel_number-1]==alpha__("MA=="):C=_D;D=0
                        else:C=_E
                ...
                if A._attr_state!=C or A._attr_brightness!=D:
                    ...
                    if A._attr_state in(_E,_D):A._stop_polling()
...
class TISRGBLight(LightEntity):
    ...
    async def async_turn_on(A,**G):
        try:
            ...
            else:logging.info(alpha__("TmVpdGhlciBjb2xvciBub3IgYnJpZ2h0bmVzcyBwcm92aWRlZCwgdXNpbmcgZGVmYXVsdCBjb2xvci4="));B=A.default_color or(0,0,0);A._attr_state=_E if A.default_color and A.default_color!=(0,0,0)else _D;A._attr_rgb_color=B;B=tuple([int(A*100/255)for A in B]);D,E,F=A.generate_rgb_packets(A,B);await A.api.protocol.sender.send_packet(D);await A.api.protocol.sender.send_packet(E);await A.api.protocol.sender.send_packet(F)
        except KeyError as H:logging.error(beta__("ZXJyb3IgdHVybmluZyBvbiBsaWdodDoge19fdmFyMH0=", __var0=H))
        A.async_write_ha_state()
    async def async_turn_off(A,**B):logging.info(alpha__("dHVybmluZyBvZmY="));logging.info(beta__("a3dhcmdzOiB7X192YXIwfQ==", __var0=B));C,D,E=A.generate_rgb_packets(A,(0,0,0));await A.api.protocol.sender.send_packet(D);await A.api.protocol.sender.send_packet(C);await A.api.protocol.sender.send_packet(E);A._attr_state=_D;A._attr_rgb_color=0,0,0;A.async_write_ha_state()
...
class TISDaliLight(LightEntity):
    ...
    async def async_added_to_hass(A):
        ...
        async def B(event):
            C=event
            if C.event_type!=str(A.device_id):return
            E=C.data[_B];F=C.data[_C]
            try:
                if E==_F:D=int(F[A.brightness_channel]);D=max(0,min(100,D));A._attr_brightness=int(D/100*255);A._attr_state=D>0;A.async_write_ha_state()
                elif E==_I:
                    G=int(C.data[_H]);B=int(F[2]);B=max(0,min(100,B))
                    if G==A.brightness_channel:A._attr_brightness=int(B/100*255);A._attr_state=B>0
                    elif G==A.temperature_channel:A._attr_color_temp_kelvin=int(A.min_color_temp_kelvin+B/100*(A.max_color_temp_kelvin-A.min_color_temp_kelvin))
                    A.async_write_ha_state()
                elif E==_G:A._attr_state=_A;A._attr_brightness=_A;A.async_write_ha_state()
            except(IndexError,KeyError,TypeError,ValueError)as H:logging.error(beta__("RXJyb3IgaGFuZGxpbmcgZXZlbnQgZm9yIERBTEkgbGlnaHQge19fdmFyMH06IHtfX3ZhcjF9", __var0=A.name, __var1=H))
        A.listener=A.hass.bus.async_listen(str(A.device_id),B)
        for C in range(5):
            if A._attr_state is not _A:break
            await A.api.protocol.sender.send_packet(A.update_packet)
        if A._attr_state is _A:A._attr_state=_A;A.async_write_ha_state()
    @property
    def brightness(self):return self._attr_brightness
    @property
    def color_mode(self):return self._attr_color_mode
    ...
    async def async_turn_on(A,**E):
        try:
            C=E.get(ATTR_COLOR_TEMP_KELVIN);B=E.get(ATTR_BRIGHTNESS)
            if B is _A:
                if A._attr_brightness is not _A and A._attr_brightness>0:B=A._attr_brightness
                else:B=255
            if C is _A:
                if A._attr_color_temp_kelvin is not _A:C=A._attr_color_temp_kelvin
                else:C=A.default_temperature
            B=max(0,min(255,int(B)));C=max(A.min_color_temp_kelvin,min(A.max_color_temp_kelvin,int(C)));F=int(B/255*100);D=int((C-A.min_color_temp_kelvin)/(A.max_color_temp_kelvin-A.min_color_temp_kelvin)*100);D=max(0,min(100,D))
            if not A.generate_dali_packets:logging.error(beta__("Tm8gREFMSSBwYWNrZXQgZ2VuZXJhdG9yIGF2YWlsYWJsZSBmb3Ige19fdmFyMH0=", __var0=A.name));return
            G,H=A.generate_dali_packets(A,F,D);I=await A.api.protocol.sender.send_packet_with_ack(G)
            if I:A._attr_brightness=B;A._attr_state=B>0
            else:logging.warning(beta__("Tm8gQUNLIHJlY2VpdmVkIGZvciBicmlnaHRuZXNzIG9mIGxpZ2h0IHtfX3ZhcjB9Lg==", __var0=A.name))
            J=await A.api.protocol.sender.send_packet_with_ack(H)
            if J:A._attr_color_temp_kelvin=C
            else:logging.warning(beta__("Tm8gQUNLIHJlY2VpdmVkIGZvciBjb2xvciB0ZW1wZXJhdHVyZSBvZiBsaWdodCB7X192YXIwfS4=", __var0=A.name))
        except Exception as K:logging.error(beta__("RXJyb3IgdHVybmluZyBvbiBsaWdodCB7X192YXIwfToge19fdmFyMX0=", __var0=A.name, __var1=K))
        A.async_write_ha_state()
    async def async_turn_off(A,**I):
        try:
            B=A._attr_color_temp_kelvin if A._attr_color_temp_kelvin is not _A else A.default_temperature;B=max(A.min_color_temp_kelvin,min(A.max_color_temp_kelvin,int(B)));C=int((B-A.min_color_temp_kelvin)/(A.max_color_temp_kelvin-A.min_color_temp_kelvin)*100);C=max(0,min(100,C))
            if not A.generate_dali_packets:logging.error(beta__("Tm8gREFMSSBwYWNrZXQgZ2VuZXJhdG9yIGF2YWlsYWJsZSBmb3Ige19fdmFyMH0=", __var0=A.name));return
            D,E=A.generate_dali_packets(A,0,C);F=await A.api.protocol.sender.send_packet_with_ack(D)
            if F:A._attr_brightness=0;A._attr_state=_D
            else:logging.warning(beta__("Tm8gQUNLIHJlY2VpdmVkIGZvciB0dXJuaW5nIG9mZiBsaWdodCB7X192YXIwfS4=", __var0=A.name))
            G=await A.api.protocol.sender.send_packet_with_ack(E)
            if G:A._attr_color_temp_kelvin=B
            else:logging.warning(beta__("Tm8gQUNLIHJlY2VpdmVkIGZvciBjb2xvciB0ZW1wZXJhdHVyZSBvZiBsaWdodCB7X192YXIwfS4=", __var0=A.name))
        except Exception as H:logging.error(beta__("RXJyb3IgdHVybmluZyBvZmYgbGlnaHQge19fdmFyMH06IHtfX3ZhcjF9", __var0=A.name, __var1=H))
        A.async_write_ha_state()