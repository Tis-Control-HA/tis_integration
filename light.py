...
W=True
V=decode__("b2ZmbGluZV9kZXZpY2U=")
U=decode__("dXBkYXRlX3Jlc3BvbnNl")
T=decode__("Y29udHJvbF9yZXNwb25zZQ==")
S=KeyError
R=range
O=decode__("Y2hhbm5lbF9udW1iZXI=")
N=tuple
M=False
K=str
H=decode__("YWRkaXRpb25hbF9ieXRlcw==")
G=iter
F=next
E=decode__("ZmVlZGJhY2tfdHlwZQ==")
A=property
C=None
B=int
import logging as D
...
async def async_setup_entry(hass,entry,async_add_devices):
    H=decode__("Z2F0ZXdheQ==");E=decode__("aXNfcHJvdGVjdGVk");D=decode__("ZGV2aWNlX2lk");C=async_add_devices;B=decode__("Y2hhbm5lbHM=");A=entry.runtime_data.api;I=await A.get_entities(platform=decode__("ZGltbWVy"))
    ...
    J=await A.get_entities(platform=decode__("cmdi"))
    ...
    K=await A.get_entities(platform=decode__("cmdidw=="))
...
class f(P):
    def __init__(A,tis_api,gateway,light_name,channel_number,device_id):A.api=tis_api;A.gateway=gateway;A.device_id=device_id;A.channel_number=B(channel_number);A._attr_name=light_name;A._attr_state=M;A._attr_brightness=C;A.listener=C;A._attr_unique_id=format_str__("e19fdmFyMH1fe19fdmFyMX0=", __var0=A.name, __var1=A.channel_number);A.setup_light()
    ...
    async def async_added_to_hass(A):
        ...
        async def C(event):
            ...
            if C.event_type==K(A.device_id):
                if C.data[E]==T:
                    D.info(format_str__("Y2hhbm5lbCBudW1iZXIgZm9yIGxpZ2h0OiB7X192YXIwfQ==", __var0=A.channel_number));F=C.data[H][2];G=C.data[O]
                    if B(G)==A.channel_number:A._attr_state=B(F)!=0;A._attr_brightness=B(F/100*255)
                    A.async_write_ha_state()
                elif C.data[E]==decode__("YmluYXJ5X2ZlZWRiYWNr"):
                    I=ceil(C.data[H][0]/8);J=decode__("").join(Z(C.data[H][A])for A in R(1,I+1))
                    if J[A.channel_number-1]==decode__("MA=="):A._attr_state=M
                    A.async_write_ha_state()
    ...
    async def async_turn_off(A,**E):
        ...
        A.async_write_ha_state()
class g(P):
    def __init__(A,tis_api,gateway,device_id,r_channel,g_channel,b_channel,light_name):A.api=tis_api;A.gateway=gateway;A.device_id=device_id;A.r_channel=B(r_channel);A.g_channel=B(g_channel);A.b_channel=B(b_channel);A.rgb_value_flags=[0,0,0];A._attr_name=light_name;A._attr_state=C;A._attr_rgb_color=C;A.listener=C;A._attr_unique_id=format_str__("e19fdmFyMH1fe19fdmFyMX1fe19fdmFyMn1fe19fdmFyM30=", __var0=A.name, __var1=A.r_channel, __var2=A.g_channel, __var3=A.b_channel);A.setup_light()
    ...
    async def async_turn_on(A,**F):
        D.info(format_str__("a3dhcmdzOiB7X192YXIwfQ==", __var0=F))
        try:
            E=F[b];E=N([B(A/255*100)for A in E]);G,H,I=A.generate_rgb_packets(A,E);D.info(format_str__("Y29sb3IgKHBlcmNlbnQpOiB7X192YXIwfQ==", __var0=E));C=await A.api.protocol.sender.send_packet_with_ack(G)
            if not C:D.error(format_str__("ZXJyb3IgdHVybmluZyBvbiBsaWdodDoge19fdmFyMH0sIGNoYW5uZWw6IHtfX3ZhcjF9", __var0=C, __var1=A.r_channel))
            C=await A.api.protocol.sender.send_packet_with_ack(H)
            if not C:D.error(format_str__("ZXJyb3IgdHVybmluZyBvbiBsaWdodDoge19fdmFyMH0sIGNoYW5uZWw6IHtfX3ZhcjF9", __var0=C, __var1=A.g_channel))
            C=await A.api.protocol.sender.send_packet_with_ack(I)
            if not C:D.error(format_str__("ZXJyb3IgdHVybmluZyBvbiBsaWdodDoge19fdmFyMH0sIGNoYW5uZWw6IHtfX3ZhcjF9", __var0=C, __var1=A.b_channel))
            A._attr_state=W;E=N([B(A/100*255)for A in E]);A._attr_rgb_color=E
        except S as J:D.error(format_str__("ZXJyb3IgdHVybmluZyBvbiBsaWdodDoge19fdmFyMH0=", __var0=J))
        A.async_write_ha_state()
...
class h(P):
    def __init__(A,tis_api,gateway,device_id,r_channel,g_channel,b_channel,w_channel,light_name):A.api=tis_api;A.gateway=gateway;A.device_id=device_id;A.r_channel=B(r_channel);A.g_channel=B(g_channel);A.b_channel=B(b_channel);A.w_channel=B(w_channel);A._attr_name=light_name;A._attr_state=C;A._attr_brightness=C;A._attr_rgbw_color=C;A.rgbw_value_flags=[0,0,0,0];A.listener=C;A._attr_unique_id=format_str__("e19fdmFyMH1fe19fdmFyMX1fe19fdmFyMn1fe19fdmFyM31fe19fdmFyNH0=", __var0=A.name, __var1=A.r_channel, __var2=A.g_channel, __var3=A.b_channel, __var4=A.w_channel);A.setup_light()
    ...
    async def async_turn_on(A,**F):
        D.info(format_str__("a3dhcmdzOiB7X192YXIwfQ==", __var0=F))
        try:
            E=F[c];E=N([B(A/255*100)for A in E]);G,H,I,J=A.generate_rgbw_packets(A,E);D.info(format_str__("Y29sb3IgKHBlcmNlbnQpOiB7X192YXIwfQ==", __var0=E));C=await A.api.protocol.sender.send_packet_with_ack(G)
            if not C:D.error(format_str__("ZXJyb3IgdHVybmluZyBvbiBsaWdodDoge19fdmFyMH0sIGNoYW5uZWw6IHtfX3ZhcjF9", __var0=C, __var1=A.r_channel))
            C=await A.api.protocol.sender.send_packet_with_ack(H)
            if not C:D.error(format_str__("ZXJyb3IgdHVybmluZyBvbiBsaWdodDoge19fdmFyMH0sIGNoYW5uZWw6IHtfX3ZhcjF9", __var0=C, __var1=A.g_channel))
            C=await A.api.protocol.sender.send_packet_with_ack(I)
            ...
            if not C:D.error(format_str__("ZXJyb3IgdHVybmluZyBvbiBsaWdodDoge19fdmFyMH0sIGNoYW5uZWw6IHtfX3ZhcjF9", __var0=C, __var1=A.w_channel))
            A._attr_state=W;E=N([B(A/100*255)for A in E]);A._attr_rgbw_color=E
        except S as K:D.error(format_str__("ZXJyb3IgdHVybmluZyBvbiBsaWdodDoge19fdmFyMH0=", __var0=K))
        A.async_write_ha_state()
