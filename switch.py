...
P=False
O=decode__("b2ZmbGluZV9kZXZpY2U=")
N=None
M=True
L=decode__("ZGV2aWNlX2lk")
A=property
K=str
I=decode__("Y2hhbm5lbF9udW1iZXI=")
H=Exception
F=int
C=decode__("ZmVlZGJhY2tfdHlwZQ==")
...
async def async_setup_entry(hass,entry,async_add_devices):
    ...
    if C:
        D=[(C,next(iter(A[decode__("Y2hhbm5lbHM=")][0].values())),A[L],A[decode__("aXNfcHJvdGVjdGVk")],A[decode__("Z2F0ZXdheQ==")])for B in C for(C,A)in B.items()]
        try:E=[W(A,B,C,D,E)for(B,C,D,F,E)in D];async_add_devices(E,update_before_add=M)
        except H as F:B.error(format_str__("ZXJyb3IgaGFwcGVuZWQgY3JlYXRpbmcgZW50aXRpZXMgZToge19fdmFyMH0=", __var0=F))
J=R()
class W(S):
    def __init__(A,tis_api,switch_name,channel_number,device_id,gateway):B=switch_name;A.api=tis_api;A._name=B;A._attr_unique_id=format_str__("c3dpdGNoX3tfX3ZhcjB9", __var0=A.name);A._state=G;A._attr_is_on=N;A.name=B;A.device_id=device_id;A.gateway=gateway;A.channel_number=F(channel_number);A.listener=N;A.on_packet=J.generate_control_on_packet(A);A.off_packet=J.generate_control_off_packet(A);A.update_packet=J.generate_control_update_packet(A)
    async def async_added_to_hass(A):
        @V
        async def J(event):
            H=decode__("YWRkaXRpb25hbF9ieXRlcw==");B=event
            if B.event_type==K(A.device_id):
                if B.data[C]==decode__("Y29udHJvbF9yZXNwb25zZQ=="):
                    J=B.data[H][2];L=B.data[I]
                    if F(L)==A.channel_number:A._state=E if F(J)==100 else D
                elif B.data[C]==decode__("YmluYXJ5X2ZlZWRiYWNr"):N=ceil(B.data[H][0]/8);P=decode__("").join(Q(B.data[H][A])for A in range(1,N+1));A._state=E if P[A.channel_number-1]==decode__("MQ==")else D
                elif B.data[C]==decode__("dXBkYXRlX3Jlc3BvbnNl"):R=B.data[H];S=F(R[A.channel_number]);A._state=E if S>0 else D
                elif B.data[C]==O:
        ...
        except H as L:B.error(format_str__("ZXJyb3IgaW4gYXN5bmNfYWRkZWRfdG9faGFzcyBmdW4gZToge19fdmFyMH0=", __var0=L))
    ...
    async def async_turn_on(A,**M):
        ...
        except H as J:B.error(format_str__("ZXJyb3IgaW4gYXN5bmNfdHVybl9vbiBlOiB7X192YXIwfQ==", __var0=J))
    ...
    async def async_turn_off(A,**M):
        ...
        except H as J:B.error(format_str__("ZXJyb3IgaW4gYXN5bmNfdHVybl9vZmYgZToge19fdmFyMH0=", __var0=J))
        A.schedule_update_ha_state()
    @A
