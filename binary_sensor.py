...
async def async_setup_entry(hass,entry,async_add_entities):
    A=entry.runtime_data.api;D=await A.get_entities(platform=alpha__("YmluYXJ5X3NlbnNvcg=="));E=await A.get_entities(platform=alpha__("cGFzc3dvcmRz"));B=[]
    if D:
        C=[(C,next(iter(A[alpha__("Y2hhbm5lbHM=")][0].values())),A[alpha__("ZGV2aWNlX2lk")],A[alpha__("Z2F0ZXdheQ==")],A[alpha__("aXNfcHJvdGVjdGVk")])for B in D for(C,A)in B.items()]
        for(F,G,H,I,J)in C:B.append(TISBinarySensor(tis_api=A,sensor_name=F,channel_number=G,device_id=H,gateway=I))
    if E:C=[(B,C)for A in E for(B,C)in A.items()];B.append(TISPasswordSensor(tis_api=A,password_name=B,password=C)for(B,C)in C)
    ...
    async def async_added_to_hass(A):
        ...
        async def B(event):
            ...
            if B.event_type==alpha__("cGFzc3dvcmRfZmVlZGJhY2s="):
                if B.data[alpha__("cGFzc3dvcmQ=")]==A._password:A._attr_is_on=_B;A.async_write_ha_state();async_call_later(A.hass,1,A._turn_off_callback)
        A._listener=A.hass.bus.async_listen(MATCH_ALL,B)
    @callback
