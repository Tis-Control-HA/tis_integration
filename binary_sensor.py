...
async def async_setup_entry(hass,entry,async_add_entities):
    ...
    if B:C=[(C,next(iter(A[alpha__("Y2hhbm5lbHM=")][0].values())),A[alpha__("ZGV2aWNlX2lk")],A[alpha__("Z2F0ZXdheQ==")],A[alpha__("aXNfcHJvdGVjdGVk")])for B in B for(C,A)in B.items()];D=[I(tis_api=A,sensor_name=B,channel_number=C,device_id=D,gateway=E)for(B,C,D,E,F)in C]
        ...
        async def handle_event(event):
            ...
            if B.event_type==str(A._device_id):
                if B.data[G]==alpha__("YXV0b19iaW5hcnlfZmVlZGJhY2s="):
                    I=B.data[alpha__("Y2hhbm5lbHNfdmFsdWVz")][A._channel_number-1]
                    if C(I)==1:A._attr_is_on=D;A._attr_state=F
                    else:A._attr_is_on=H;A._attr_state=E
                elif B.data[G]==alpha__("cmVhbHRpbWVfZmVlZGJhY2s="):
                    if B.data[alpha__("Y2hhbm5lbF9udW1iZXI=")]==A._channel_number:
                        J=C(B.data[alpha__("YWRkaXRpb25hbF9ieXRlcw==")][1])
